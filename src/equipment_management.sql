...
--        AVG((strftime('%s', 'now') - strftime('%s', purchase_date)) / (60 * 60 * 24.0)) AS avg_age_days  -- uses unix timesramp to calulate difference in age and todays date then converts to avg days
-- FROM equipment  
-- GROUP BY type;